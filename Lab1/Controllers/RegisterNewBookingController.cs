    ...
    public class RegisterNewBookingController : Controller
    {
        ...
        public ActionResult ChooseDate([Bind(Include = "Settling,Eviction,ValueOfGuests,ValueOfKids,ClientId")] Booking booking)
        {
            if (ModelState.IsValid)
            {
                ...
                return View("~/Views/RegisterNewBooking/AvailableApartments.cshtml", apartments);
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult FinishTheRegistration([Bind(Include = "Id,Number,Price,ApartmentTypeId")] Apartments apartments, int BookingId, int ClientId, DateTime Settling, DateTime Eviction, int ValueOfGuests, int ValueOfKids)
        {
            Apartments currentApartment = db.Apartments.Include(a => a.ApartmentType).Where(x => x.Id == apartments.Id).First(); 
